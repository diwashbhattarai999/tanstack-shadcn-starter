...
import { selectHasPermission, useAuthStore } from "@/stores/auth-store";
	...
	const filteredItems = items.filter((item) => {
		...
		if (Array.isArray(item.permission)) {
			return item.permission.some((perm) =>
				selectHasPermission(authState, perm)
			);
		}
		return selectHasPermission(authState, item.permission);
	});
	...
	return (
		<SidebarMenuItem>
			<SidebarMenuButton
				...
				render={<Link onClick={() => setOpenMobile(false)} to={item.url} />}
				tooltip={item.title}
			>
				{item.icon && <item.icon />}
